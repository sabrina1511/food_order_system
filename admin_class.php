...
Class Action {
	...
	function save_food(){
		...
		$check = $this->db->query("SELECT * FROM foods where name ='$name' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO foods set $data");
		}else{
			$save = $this->db->query("UPDATE foods set $data where id = $id");
		}
	...
	function delete_food(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM foods where id = ".$id);
		if($delete){
			return 1;
		}
	}
